...
public class GoogleCalendarVerifierExtension extends DefaultComponentVerifierExtension {
	...
	protected Result verifyConnectivity(Map<String, Object> parameters) {
		...
		try {
			GoogleCalendarStreamConfiguration configuration = setProperties(new GoogleCalendarStreamConfiguration(),
					parameters);
			GoogleCalendarClientFactory clientFactory = new BatchGoogleCalendarClientFactory();
			Calendar client = clientFactory.makeClient(configuration.getClientId(), configuration.getClientSecret(),
					configuration.getScopes(), configuration.getApplicationName(), configuration.getRefreshToken(),
					configuration.getAccessToken(), null, null, "me");
			...
			ResultErrorBuilder errorBuilder = ResultErrorBuilder
					...
					.detail("google_calendar_exception", e.getMessage())
